...
// nome do arqv modificado para .dat (data file)
...
void criar_pasta_data_se_nao_existir() {
        ...
        _mkdir(DATA_DIR);
    #else
        mkdir(DATA_DIR, 0777); // permissão de leitura/escrita/execução
    #endif
}

// funções CRUD que foram adaptadas p/ binário:
...
void cadastrar_cliente(void) {
    ...
    // define o novo cliente como ativo por padrão
    novo_cliente.ativo = 1;

    // abre o arquivo em modo append binary 
    ...
    // escreve a struct inteira de uma vez no arquiv
...
// pesquisar por um cliente no arquivo binário usando o CPF
...
void pesquisar_cliente(void) {
    ...
    while (fread(&cliente_lido, sizeof(Cliente), 1, arq_clientes)) {
        if (strcmp(cliente_lido.cpf, cpf_pesquisa) == 0 && cliente_lido.ativo == 1) {
            ...
            printf("Email: %s\n", cliente_lido.email);
            encontrado = 1;
            break; // serve p/ parar o loop assim que encontra
        }
    }
    fclose(arq_clientes);
...
void alterar_cliente(void) {
    ...
    Cliente cliente_lido;
    FILE* arq_clientes;
    long int pos; // variável para guardar a posição do registro no arquivo
    ...
    // abre em modo leitura e escrita binária
    ...
    while (fread(&cliente_lido, sizeof(Cliente), 1, arq_clientes)) {
        if (strcmp(cliente_lido.cpf, cpf_alteracao) == 0 && cliente_lido.ativo == 1) {
            // guarda a posição do inicio do registro
            ...
            break; // sai do loop pois já encontrou e alterou
        }
    }
    fclose(arq_clientes);
...
// Realiza a "exclusão lógica" de um cliente marcando seu campo ativo" como 0
...
// lê o arquivo binário e exibe todos os clientes marcados como ativos (=1)

void listar_clientes(void) {
    FILE* arq_clientes;
    Cliente cliente_lido;
    int nenhum_encontrado = 1;
...
//função principal que exibe o menu do módulo de clientes e gerencia 
